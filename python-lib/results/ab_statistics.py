...
class AbStatistics(object):
    ...
    def compute(self, results):
        ...
        groups = list(set(results_df[self.group_column]))
        self.check_results_df(results_df, groups)
        aggregation = self.aggregate_by_group(results_df)
        statistics_df = self.format_statistics(aggregation, groups)
    ...
    def check_results_df(self, valid_rows, groups):
        ...
        if len(groups) != 2:
            raise ValueError("The group column indicator should be binary")
    ...
    def format_statistics(self, aggregation, groups):
        rows = []
        for group in groups:
    ...
    def statistic_per_group(self, group, aggregation):
        ...
        row = {self.group_column: group, Columns.SUCCESS_RATE: self.conversion_rate(group_aggregation, size), Columns.SIZE: size}
        return row
